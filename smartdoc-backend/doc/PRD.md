...
| V1.1     | 会话管理 + 工具增强 | F-02 对话历史, F-03 清除会话, F-06/F-07 工具对接真实数据源 | **[假设]** 2026-Q3 | 规划中 |
...
| 模块 | 功能ID | 功能           | 优先级 | 版本   | 说明                      |
|----|------|--------------|-----|------|-------------------------|
| 对话 | F-01 | 智能对话（SSE 流式） | P0  | V1.0 | 核心功能，流式返回 AI 回复         |
| 对话 | F-02 | 查看对话历史       | P1  | V1.1 | 按 sessionId 查询历史消息      |
| 对话 | F-03 | 清除会话         | P2  | V1.1 | 删除指定会话的对话记忆             |
| 文档 | F-04 | 上传文档         | P0  | V1.0 | 支持 PDF/DOCX/TXT，自动解析向量化 |
| 文档 | F-05 | 已上传文档列表      | P1  | V1.0 | 返回所有已上传文档文件名清单          |
| 工具 | F-06 | 知识库搜索工具      | P2  | V1.1 | Agent 工具，当前为 stub       |
| 工具 | F-07 | 业务办理状态查询工具   | P2  | V1.1 | Agent 工具，当前为 stub       |
...
| 元素 | 说明                                     |
|----|----------------------------------------|
| 方法 | `DELETE /api/chat/session/{sessionId}` |
| 响应 | `AjaxResult`                           |
...
| 指标                 | 目标                     | 当前实测值     |
|--------------------|------------------------|-----------|
| SSE 首 token 时间     | < 3s（含 LLM + RAG 检索）   | **[待确认]** |
| 并发 SSE 连接数         | 支持 20 个并发连接            | **[待确认]** |
| 文档上传解析（10MB PDF）   | < 10s 完成解析 + 向量化       | **[待确认]** |
| 对话历史查询             | < 500ms                | **[待确认]** |
