...
Transform the plain-Java LangChain4j application (Step 06) into a produciton-ready Spring Boot multi-module web service
with REST and SSE endpoints.
...
For Step 08 (Vue frontend), we use `fetch()` + `ReadableStream` instead of `EventSource` because we need `POST` to send
the chat message.

### Provider Switching

The `LlmConfig` reads the active provider from configuration and creates the appropriate `ChatModel` /
...
[Step 08: Vue 3 Frontend](../smartdoc-frontend/) — Build a browser-based chat UI that consumes this API
